...
def usershell(user, name, *args):
    """
    Open a shell in a docker container tagged with liquid_task=`name` as 'user'
    """
    containers = docker.containers([('liquid_task', name)])
    id = first(containers, 'containers')
    docker_exec_cmd = ['docker', 'exec', '-it', '--user', user , id] + list(args or ['bash'])
    run_fg(docker_exec_cmd, shell=False)
