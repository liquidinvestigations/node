  ...
  group "nextcloud27" {
    ...
    task "nextcloud27" {
      ...
      template {
        ...
        NEXTCLOUD_IP = "http://{{ env "attr.unique.network.ip-address" }}:${config.port_nextcloud27}"
        ONLYOFFICE_IP = "http://{{ env "attr.unique.network.ip-address" }}:${config.port_onlyoffice}"
