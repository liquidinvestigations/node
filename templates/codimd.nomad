...
job "codimd" {
  ...
  group "codimd" {
    ...
    task "codimd" {
      ...
      env {
        __GIT_TAGS = "${liquidinvestigations_codimd_server_git}"
        ...
        CMD_PROTOCOL_USESSL = "{% if config.https_enabled or config.liquid_http_protocol == 'https' %}true{% else %}false{% endif %}"
