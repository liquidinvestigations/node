      ...
      template {
        data = <<EOF
          ...
          {{- range service "vmck-imghost" }}
            QEMU_IMAGE_URL = "http://{{.Address}}:{{.Port}}/bionic-vagrant-2.qcow2"
          {{- end }}
        EOF
  ...
  group "drone" {
    task "drone" {
      ...
      env {
        ...
        #DRONE_RUNNER_ENVIRON="TEST:1,VMCK_IP:10.66.60.1,VMCK_PORT:9999"
      }
      template {
        data = <<EOF
          {{- range service "vmck" }}
            DRONE_RUNNER_ENVIRON="TEST:1,VMCK_IP:{{.Address}},VMCK_PORT:{{.Port}}"
          {{- end }}
        EOF
