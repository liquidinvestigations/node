...
INSERT INTO public.authentication (key, "isEnabled", config, "selfRegistration", "domainWhitelist", "autoEnrollGroups", "order", "strategyKey", "displayName") SELECT 'liquid', true, '{"clientId":"$WIKIJS_OAUTH2_CLIENT_ID","clientSecret":"$WIKIJS_OAUTH2_CLIENT_SECRET","authorizationURL":"$WIKIJS_OAUTH2_AUTHORIZATION_URL","tokenURL":"$WIKIJS_OAUTH2_TOKEN_URL","userInfoURL":"$WIKIJS_OAUTH2_USER_PROFILE_URL","userIdClaim":"$WIKIJS_OAUTH2_USER_PROFILE_ID_ATTR","displayNameClaim":"$WIKIJS_OAUTH2_USER_PROFILE_USERNAME_ATTR","emailClaim":"$WIKIJS_OAUTH2_USER_PROFILE_EMAIL_ATTR","mapGroups":true,"groupsClaim":"$WIKIJS_OAUTH2_USER_PROFILE_GROUPS_ATTR","logoutURL":"$WIKIJS_OAUTH2_LOGOUT_URL","scope":"read","useQueryStringForAccessToken":false,"enableCSRFProtection":true}', true, '{"v":[]}', '{"v":[2]}', 0, 'oauth2', '$WIKIJS_OAUTH2_PROVIDERNAME' WHERE NOT EXISTS (SELECT key FROM public.authentication WHERE key = 'liquid');
